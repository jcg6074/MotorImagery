...
from keras.layers import Add, Concatenate, Lambda, Input, Permute, Reshape
...
def ATCNET(n_classes, in_chans=62, in_samples=265, n_windows=5, attention=None,
           eegn_F1=16, eegn_D=2, eegn_kernelSize=32, eegn_poolSize=8, eegn_dropout=0.3,
           tcn_depth=2, tcn_kernelSize=4, tcn_filters=32, tcn_dropout=0.3,
           tcn_activation='elu', fuse='concat'):
    ...
    """
    input = Input(shape=(in_samples,in_chans,5))
    regRate = .25
    numFilters = eegn_F1
    F2 = numFilters * eegn_D

    block1 = Conv_block(input_layer=input, F1=eegn_F1, D=eegn_D,
                        kernLength=eegn_kernelSize, poolSize=2,
                        in_chans=in_chans, dropout=eegn_dropout)
    #block1 = tf.math.reduce_mean(block1, axis=2, keepdims=True)
    ...
    if (fuse == 'average'):
        ...
        last = Dense(n_classes, kernel_constraint=max_norm(regRate))(sw_concat)
    decoder = Decoder(sw_concat)
    softmax = Activation('softmax', name='softmax')(last)

    return Model(inputs=input, outputs=[softmax,decoder])


# %% Convolutional (CV) block used in the ATCNet model
def Conv_block(input_layer, F1=4, kernLength=64, poolSize=2, D=2, in_chans=22, dropout=0.1):
    """ Conv_block
    ...
    """
    F2 = F1 * D
    block1 = Conv2D(F1, (kernLength, 1), padding='same', data_format='channels_last', kernel_initializer="glorot_uniform",use_bias=False)(input_layer)
    ...
    block2 = AveragePooling2D((6, 1), data_format='channels_last')(block2)
    block2 = Dropout(dropout)(block2)
    block3 = Conv2D(F2, (8, 1),
                    data_format='channels_last',
...
def TCNet_Fusion(n_classes, Chans=22, Samples=1125, layers=2, kernel_s=4, filt=12,
    ...
    EEGNet_sep = EEGNet(input_layer=input, F1=F1, kernLength=kernLength, D=D, Chans=Chans, dropout=dropout_eeg)
    ...
    #dense1 = Dense(100)(Con2)
    dense = Dense(n_classes, kernel_constraint=max_norm(regRate))(Con2)
    softmax = Activation('softmax', name='softmax')(dense)
    decoder = Decoder(input_layer=dense)
...
def Decoder(input_layer):
    de_conv = tf.expand_dims(input_layer, axis=1)
    de_conv = Conv1D(filters=1125, kernel_size=4, padding="same", activation='elu',strides=1, data_format='channels_first')(de_conv)
    de_conv = attention_block(de_conv, "mha")
    de_conv = tf.expand_dims(de_conv, axis=1)
    de_conv = Conv2D(filters=22, kernel_size=(1,4), activation='elu',padding="valid",strides=1, data_format='channels_first')(de_conv)
    de_conv = tf.transpose(de_conv, [0, 2, 1, 3])
